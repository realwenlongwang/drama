...
class MixerModel(nn.Module):
    def __init__(
        ...
        self.stem = nn.Sequential(
            nn.Linear(stoch_dim+action_dim, d_model, bias=True),
            _RMSNorm(d_model),
            nn.SiLU(inplace=True),
            # nn.Linear(d_model, d_model, bias=True),
            # _RMSNorm(d_model) # output layer normalise the hidden state
        )
...
class MambaWrapperModel(nn.Module, GenerationMixin):
    ...
    #     self.tie_weights()
    # TODO: Investigate if tie weights can work in the encoder decoder 
